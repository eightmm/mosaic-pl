#!/usr/bin/env bash
#SBATCH --job-name=rerun_ph
#SBATCH --output=/home/jaemin/project/CASP17/experiments/logs/rerun_ph-%A_%a.out
#SBATCH --error=/home/jaemin/project/CASP17/experiments/logs/rerun_ph-%A_%a.err
#SBATCH --gres=gpu:1
#SBATCH --mem=48G
#SBATCH --time=02:00:00
#SBATCH --partition=heavy
#SBATCH --array=186-372%3
...
# Skip if another task already produced a recent submission (within last 20min)
if [ -f "$LG_OUT" ] && [ "$(find "$LG_OUT" -mmin -20 2>/dev/null)" ]; then
    echo "=== skip $RUN_NAME (recent LG exists) ==="
    exit 0
fi
