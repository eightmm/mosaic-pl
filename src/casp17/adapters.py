...
def _prepare_autodock_gpu_variant(
    ...
    script_lines = [
        ...
        "    return types",
        "",
        "# Same parser, different alias — used for the receptor pdbqt so",
        "# nucleic-acid receptors (P from phosphate, modified atom types)",
        "# and metal-bearing receptors (Mg/Zn/Ca/Fe/Mn from cif_to_pdb's",
        "# retain pass) get their atom types into receptor_types. Without",
        "# this autogrid4 emits a `WARNING: receptor type X not in list`",
        "# and silently drops those atoms from the elec/dsolv map.",
        "def _parse_rec_types(path):",
        "    return _parse_lig_types(path)",
        "",
        ...
        "    lig_receptor = lig.get('receptor_pdbqt') or receptor_pdbqt",
        "    # autogrid is invoked with cwd=lig_grid_dir, so any relative path",
        "    # in prep summary resolves to the wrong place. Force absolute.",
        "    lig_receptor = str(Path(lig_receptor).resolve())",
        "    lig_pdbqt = str(Path(lig_pdbqt).resolve())",
        "    lig_out_dir = seed_out_dir / f'ligand_{lig_id}'",
        "    lig_out_dir.mkdir(parents=True, exist_ok=True)",
        ...
        "    except Exception as exc:",
        "        # Surface the actual autogrid error rather than swallowing —",
        "        # silent ADG failures left template_consensus_* with zero",
        "        # poses across the 499-target batch on 2026-05-09.",
        "        print(f'  -> {lig_id}: AutoDock-GPU failed: {exc}', file=sys.stderr)",
        "        autogrid_log = lig_grid_dir / 'autogrid.log'",
        "        if autogrid_log.exists():",
        "            tail = autogrid_log.read_text().splitlines()[-20:]",
        "            print('  -> autogrid.log tail:', file=sys.stderr)",
        "            for line in tail:",
        "                print(f'      {line}', file=sys.stderr)",
        "        (lig_out_dir / 'FAILED').write_text(str(exc) + '\\n')",
        "        continue",
    ]
    runner_script.write_text("\n".join(script_lines) + "\n")
...
def prepare_surfdock(
    common: CommonInput, config: RunnerConfig, run_dir: Path
) -> PreparedModelRun | None:
    """Wire SurfDock into the docking stage as a 4th pose-diversity lane.

    SurfDock is a three-step pipeline (surface mesh, ESM-2 3B pocket
    embedding, diffusion inference); we drive all three from a single bash
    runner per target. The runner reads the docking-prep summary at
    runtime, picks the receptor PDB + ligand SDF for the
    ``cofolding_1`` binding site (top cofold cluster), and emits SurfDock
    poses under ``outputs/surfdock/``.

    A single PreparedModelRun is sufficient — SurfDock's diffusion
    sampling itself produces ``samples_per_complex`` (default 40) poses
    per call, so we get pose diversity without a 6-source fan-out (which
    Vina/AutoDock-GPU need because they're single-pose searches).
    """
    if not config.surfdock.enabled:
        return None

    output_dir = run_dir / "outputs" / "surfdock"
    output_dir.mkdir(parents=True, exist_ok=True)
    surface_dir = output_dir / "surface"
    embed_dir = output_dir / "embed"
    runner_script = run_dir / "scripts" / "run_surfdock.sh"
    runner_script.parent.mkdir(parents=True, exist_ok=True)
    notes: list[str] = []

    summary_path = run_dir / "inputs" / "docking" / "docking_prep_summary.json"
    repo_dir = config.surfdock.repo_dir
    python_bin = config.surfdock.python_bin
    model_dir = config.surfdock.model_dir
    confidence_dir = config.surfdock.confidence_model_dir

    body = [
        "#!/usr/bin/env bash",
        "# SurfDock runner — surface mesh + ESM-2 3B pocket embedding +",
        "# diffusion inference. Generated by casp17.adapters.prepare_surfdock.",
        "set -euo pipefail",
        "",
        f"REPO_DIR={shlex.quote(str(Path(repo_dir).resolve()))}",
        f"PYTHON_BIN={shlex.quote(python_bin)}",
        f"SUMMARY_PATH={shlex.quote(str(summary_path))}",
        f"OUT_DIR={shlex.quote(str(output_dir))}",
        f"SURFACE_DIR={shlex.quote(str(surface_dir))}",
        f"EMBED_DIR={shlex.quote(str(embed_dir))}",
        f"MODEL_DIR={shlex.quote(model_dir)}",
        f"CONF_MODEL_DIR={shlex.quote(confidence_dir)}",
        f"INFER_STEPS={config.surfdock.inference_steps}",
        f"BATCH_SIZE={config.surfdock.batch_size}",
        f"BATCH_MOL={config.surfdock.batch_size_molecule}",
        f"SAMPLES_PER_COMPLEX={config.surfdock.samples_per_complex}",
        f"POCKET_R={config.surfdock.pocket_cutoff_a}",
        f"NUM_RUNS={config.surfdock.num_inference_runs}",
        f"DEVICES={config.surfdock.devices}",
        "",
        "if [ ! -f \"$SUMMARY_PATH\" ]; then",
        "    echo 'SurfDock: docking_prep_summary.json missing — skip'; exit 0",
        "fi",
        "",
        "mkdir -p \"$OUT_DIR\" \"$SURFACE_DIR\" \"$EMBED_DIR\"",
        "",
        "# Extract receptor PDB + ligand SDF + cofolding_1 box centre.",
        "RECEPTOR_PDB=$(\"$PYTHON_BIN\" -c \"import json,sys; d=json.load(open(r'$SUMMARY_PATH')); print(d['receptor_pdb'])\")",
        "LIGAND_SDF=$(\"$PYTHON_BIN\" -c \"import json,sys; d=json.load(open(r'$SUMMARY_PATH')); ls=d.get('ligands') or []; print(ls[0]['sdf'] if ls else '')\")",
        "if [ -z \"$RECEPTOR_PDB\" ] || [ -z \"$LIGAND_SDF\" ]; then",
        "    echo 'SurfDock: receptor/ligand missing in prep summary — skip'; exit 0",
        "fi",
        "",
        "# Step 1: target surface mesh (computeTargetMesh_test_samples.py).",
        "echo '[surfdock] step 1 — target surface mesh'",
        "\"$PYTHON_BIN\" \"$REPO_DIR/comp_surface/prepare_target/computeTargetMesh_test_samples.py\" \\",
        "    --data_dir \"$(dirname \"$RECEPTOR_PDB\")\" \\",
        "    --out_dir \"$SURFACE_DIR\"",
        "",
        "# Step 2: ESM-2 3B pocket embedding.",
        "echo '[surfdock] step 2 — ESM-2 pocket embedding'",
        "\"$PYTHON_BIN\" \"$REPO_DIR/comp_surface/prepare_target/esm_embedding_PDBBind.py\" \\",
        "    --pdb_dir \"$(dirname \"$RECEPTOR_PDB\")\" \\",
        "    --out_dir \"$EMBED_DIR\" \\",
        "    --pocket_cutoff \"$POCKET_R\" || true",
        "",
        "# Step 3: SurfDock diffusion inference (inference_accelerate.py).",
        "echo '[surfdock] step 3 — diffusion inference'",
        "INPUT_CSV=\"$OUT_DIR/input.csv\"",
        "\"$PYTHON_BIN\" -c \"",
        "import csv,sys,json",
        "summary=json.load(open(r'$SUMMARY_PATH'))",
        "rec=summary['receptor_pdb']; lig=summary['ligands'][0]['sdf']",
        "with open(r'$INPUT_CSV','w',newline='') as fh:",
        "    w=csv.writer(fh); w.writerow(['complex_name','protein_path','ligand'])",
        "    w.writerow(['target', rec, lig])",
        "\"",
        "for run in $(seq 1 $NUM_RUNS); do",
        "    \"$PYTHON_BIN\" \"$REPO_DIR/inference_accelerate.py\" \\",
        "        --data_csv \"$INPUT_CSV\" \\",
        "        --model_dir \"$MODEL_DIR\" \\",
        "        --confidence_model_dir \"$CONF_MODEL_DIR\" \\",
        "        --surface_path \"$SURFACE_DIR\" \\",
        "        --esm_embeddings_path \"$EMBED_DIR\" \\",
        "        --out_dir \"$OUT_DIR/run_${run}\" \\",
        "        --inference_steps \"$INFER_STEPS\" \\",
        "        --batch_size \"$BATCH_SIZE\" \\",
        "        --batch_size_molecule \"$BATCH_MOL\" \\",
    ...
    notes.append(
        "SurfDock requires conda env (Python 3.10 + PyTorch 2.2.2 + PyG) "
        "and ESM-2 3B model cache. See external/SurfDock/README.md."
    )

    cmd = ["bash", str(runner_script)]
    return PreparedModelRun("surfdock", runner_script, output_dir, cmd, notes)
