...
- **Template search union**: mmseqs2 (sequence, `min-seq-id 0.3 / cov 0.7 / sens 7.5 / max-seqs 200`) and foldseek (structure, `easy-search -s 9.5 --max-seqs 500`) run as **independent stages**, then `template_filter.filter_hits_with_ligands` union-merges by `(pdb_id, chain_id)`. **Tanimoto/MCS are metadata only here** — every hit with a candidate ligand survives. The MCS threshold gates **only** Track 3 (lig-align) where atoms must actually overlay.
- **Foldseek same-fold floor**: foldseek has no native identity/coverage gate (unlike mmseqs), so a `qtmscore_min` floor (default 0.5 — canonical Zhang/Skolnick "same fold") is applied to **foldseek-only hits**, comparing against `max(qtmscore, ttmscore)` so a small template covering a query domain (high qtm) and a big template whose domain is our query (high ttm) both qualify. mmseqs hits and dual-source hits bypass this gate. Below 0.5 the binding-site equivalence assumption breaks down, so pocket extraction would just add noise.
...
- **Alignment quality gate**: `extract_template_pockets.py` uses `gemmi.calculate_superposition` which is *sequence-anchored* (Needleman–Wunsch on protein letters then Kabsch on matched CAs). For foldseek-only hits with shared fold but very different sequence, the matched-residue set collapses and the resulting transform misplaces ligand centroids by tens of Å. Two cheap drops protect downstream clustering: `--max-alignment-rmsd` (default 5.0 Å) and `--min-aligned-residues` (default 50). Tested case: `101m` (myoglobin) injected as foldseek-only hit against a 7htl-style query → rmsd 16.7 Å, only 34 CAs aligned → correctly dropped before clustering.
