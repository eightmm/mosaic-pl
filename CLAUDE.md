...
unified YAML → adapters.py → model-specific inputs → orchestrator.py → SLURM scripts
       ...
       template-search-sequence (MMseqs2)         cofolding (5 seeds × 5 samples per model)
                  ...
                  extract template pockets (USalign each → bound-ligand centroids in cofold frame)
                                        ↓
                  cluster pockets (greedy first-match centroid, 5 Å cutoff) → top-K consensus centroids
                                        ↓
...
- **AF3 unified MSA pipeline** (default ON, `msa_pipeline.enabled=true`): `script_builder._emit_msa_pipeline_bridge` runs AF3 `run_alphafold.py --run_data_pipeline=true --run_inference=false` once per target (jackhmmer + hmmsearch + template search; ~30-90 min CPU), then `bridge_distribute_msa_templates.py` writes the unpaired/paired MSA to `shared_msa/{chain}_{kind}.a3m`, materialises inline `templates[*].mmcif` blobs into `shared_msa/templates/<pdb>.cif`, and patches Boltz YAML / Protenix JSON / AF3 input so all three cofolding tools consume the same homologs and cif templates. Adapter guards force `boltz.use_msa_server=False`, `protenix.msa_server_mode="none"`, `alphafold3.run_data_pipeline=False` whenever the pipeline is enabled. Validated 2026-04-30 on 7hqq (4 templates: 5X04 / 2ESS / 2OWN / 4GAK; ~3.7 M-char unpaired + 3.6 M-char paired).
- **Boltz MSA → AF3 (legacy fallback)**: `scripts/bridge_boltz_msa_to_af3.py` — fires only when `msa_pipeline.enabled=false`; CSV→A3M + patch AF3 JSON (pairedMsa="" + templates=[]).
...
- **Template Pocket Clustering**: `scripts/cluster_template_pockets.py` — greedy first-match clustering on running weighted centroid (each new pocket joins the *first* cluster within `--cutoff` Euclidean, default 5 Å; otherwise spawns a new cluster). Order-sensitive: extract step pre-sorts by evidence so strong hits seed clusters. *Not* Lance-Williams single-link. Per-pocket weight = `(in_mmseqs + in_foldseek) + max(qtmscore, pident/100)`; cluster `evidence_score = Σ weight`. Top-K (default 5) saved to `template_pocket_clusters.json` for downstream consumers.
...
- **novel2025 sequence redundancy**: 499 input targets collapse to **246 unique enzymes** at 100 % sequence identity (mmseqs `easy-cluster`); a single 130-member cluster (XChem fragment screen on `9s4h`) accounts for 26 % of the dataset. **Headline SR should use `cluster_rep_only @ 100%` not raw per-target average** — the XChem cluster's small/low-affinity fragments drag per-target SR down by 6-8 pp. Tooling: `scripts/cluster_novel2025_targets.py` writes `cluster_targets_<NNN>.csv` (target → rep + size); `scripts/analyze_sr_by_cluster.py` produces `sr_by_cluster.csv` (long-format SR across `per_target`/`cluster_rep_only`/`cluster_any`/`cluster_mean` × novel/remote/related zones × thresholds). Detailed numbers + interpretation in `experiments/novel2025_test/README.md` + `docs/pose_ranker_design.md`.
